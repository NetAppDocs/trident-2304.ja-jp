...
[%collapsible]
====
NetApp Consoleを使用するAstra Tridentでは、最新バージョンのAstra Tridentにアップグレードしたり、ストレージ クラスを追加および管理して作業環境に接続したり、 Cloud Backup Serviceを使用して永続ボリュームをバックアップしたりできます。コンソールは、手動または Helm を使用して、 Tridentオペレーターを使用したAstra Trident のデプロイメントをサポートします。
